...
function getDispAnimationDuration(){
	let cssInfo = document.querySelector(`#css-info-container`)
	const ANIMATION_DURATION = getComputedStyle(cssInfo).animationDuration
	console.log(ANIMATION_DURATION)
}

function animateDisplay(){
	const DURATION = getDispAnimationDuration()
	display.classList.remove(`display-full`)
	display.classList.add(`display-full`)
	setTimeout(() => display.classList.remove(`display-full`), 400);
}
